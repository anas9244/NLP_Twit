    ...
    for i in range(1, iters + 1):
        ...
        for cluster in clusters_tweets:
            ...
            count_vectorizer = CountVectorizer(
                preprocessor=lambda x: x, strip_accents=False, lowercase=False)
            tf_vectorizer = TfidfTransformer(use_idf=False, norm="l1")
            start = time.time()
            X = count_vectorizer.fit_transform([sample_corpus])
            elapsed_time = time.time() - start
            print("vectorizer done after: ", elapsed_time)
            start = time.time()
            merged_X = np.sum(X, 0)
            elapsed_time = time.time() - start
            print("merging done after: ", elapsed_time)
        ...
        print("sample_mean_perplexity: ", sample_mean_perplexity)
...
def perplexity(gran, nMin, nMax, algo, distance_type, method='ward', metric='norm'):
    ...
    for n in range(nMin, nMax):
        ...
        print(mean_perplexity / n)
        results.append(mean_perplexity / n)
